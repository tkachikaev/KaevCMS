...
try {
    ...
    Assert-True ($updateScript.Contains("$recoverableFromVersions = @('0.34.8')")) 'Updater does not recover an interrupted 0.34.9 update from 0.34.8.'
    Assert-True ($updateScript.Contains("$supersededPendingTargets = @('0.34.9')")) 'Updater does not adopt the interrupted 0.34.9 pending target.'
    ...
    $applyScript = Get-Content -LiteralPath "$PSScriptRoot\..\apply-0.35.0.ps1" -Raw
