...
try {
    ...
    Assert-True ($updateScript.Contains("$recoverableFromVersions = @('0.33.8')")) 'Updater does not recover an interrupted 0.33.9 update from 0.33.8.'
    Assert-True ($updateScript.Contains("$supersededPendingTargets = @('0.33.9')")) 'Updater does not adopt the interrupted 0.33.9 pending target.'
    ...
    $applyScript = Get-Content -LiteralPath "$PSScriptRoot\..\apply-0.34.0.ps1" -Raw
