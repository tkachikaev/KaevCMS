...
The bundled `promo-codes` module grants server-bound rewards through the core web inventory. The bundled `daily-rewards` module provides monthly, server-specific reward calendars with one claim per eligible game account and immutable claim history. The bundled `support-tickets` module provides private player tickets, staff replies, internal notes, assignment and revision history.
